...
void replace(std::string *line, std::string r, std::string r_w)
{
    size_t start_pos;

    while ((start_pos = (*line).find(r)) != std::string::npos)
    {
        (*line).erase(start_pos, r.length());
...
int main(int argc, char **argv)
{
    ...
    {
        replace(&line_read, to_replace, replace_with);
        if (!std::cin.eof())
